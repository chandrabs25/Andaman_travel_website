...
import { MapPin, Clock, Star, ArrowRight, Filter, X, Loader2 } from 'lucide-react';
    ...
    return (
        <div className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col transition-transform duration-300 hover:shadow-lg hover:scale-[1.02]">
            <div className="h-48 w-full relative flex-shrink-0">
                ...
                />
               {/* --- FIX: Use HTML entity for Rupee symbol --- */}
            ...
            <div className="p-4 flex flex-col flex-grow">
               <div className="flex justify-between items-start mb-1">
                 ...
                 <div className="flex items-center text-yellow-500 flex-shrink-0 ml-2"> <Star size={14} fill="currentColor" /> <span className="ml-1 text-xs font-medium">{pkg.rating}</span> </div>
               </div>
...
export default function Packages() {
  ...
  const [loading, setLoading] = useState(false); // Assuming mock data, set initial loading to false
  ...
  // --- Handlers and Effects ---
    ...
    useEffect(() => {
        // Since we are using mock data, apply filters immediately
        applyFilters();
        setLoading(false); // Set loading to false as data is static
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [filters, packages]); // Re-run if filters or base packages change
                      ...
                      <div>
                          ...
                          <div className="flex items-center border border-gray-300 rounded-md focus-within:ring-1 focus-within:ring-blue-500 focus-within:border-blue-500 bg-white overflow-hidden">
                             ...
                             <select id="destination" name="destination" value={filters.destination} onChange={handleFilterChange} className="pl-2 pr-8 py-2 w-full text-sm border-none focus:ring-0 bg-transparent appearance-none" style={{ backgroundImage: 'none' }}>
                                <option value="">All</option> <option value="Port Blair">Port Blair</option> <option value="Havelock">Havelock</option> <option value="Neil">Neil Island</option> <option value="Baratang">Baratang</option> <option value="Diglipur">Diglipur</option>
                             </select>
                          </div>
                      </div>
                      {/* Duration Filter */}
                      ...
                      {/* Price Range Filter */}
                       <div>
                           ...
                           <select id="priceRange" name="priceRange" value={filters.priceRange} onChange={handleFilterChange} className="pl-3 pr-8 py-2 w-full border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 bg-white appearance-none text-sm" style={{ backgroundImage: 'none' }}>
                             <option value="">Any Price</option> <option value="10000-15000">₹10k - ₹15k</option> <option value="15000-20000">₹15k - ₹20k</option> <option value="20000-25000">₹20k - ₹25k</option> <option value="25000-30000">₹25k - ₹30k</option> <option value="30000">₹30k+</option>
                           </select>
                       </div>
                      {/* Activities Filter */}
                      <div>
